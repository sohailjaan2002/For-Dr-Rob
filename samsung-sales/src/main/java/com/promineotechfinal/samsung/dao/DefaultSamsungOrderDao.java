...
SqlParams params = generateInsertSql(customer, samsung, color, processor, battery, price); 
  ...
  class ModelResultSetSxtractor implements ResultSetExtractor<Samsung> {
    ...
    public Samsung extractData(ResultSet rs) throws SQLException, DataAccessException {
      rs.next();
      return Samsung.builder().launchPrice(rs.getBigDecimal("launch_price"))
          .modelPK(rs.getLong("model_pk")).storageInGB(rs.getInt("storage_in_GB"))
          .typeModel(rs.getString("type_model")).screenSize(rs.getInt("screen_size")).build();
    }
  ...
  @Override
  public Optional<Processor> fetchProcessor(@NotNull String processorId) {
    //@formatter: off
    String sql = ""
        + "SELECT * "
        + "FROM processors "
